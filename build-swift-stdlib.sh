...
RUNTIME_FLAGS="-w -fuse-ld=lld --sysroot=${STAGING_DIR} -target ${SWIFT_TARGET_NAME} -march=armv7-a -mthumb -mfpu=neon -mfloat-abi=hard -B${STAGING_DIR}/usr/lib/c++/10 -B${STAGING_DIR}/usr/lib -B${STAGING_DIR}/lib"
LINK_FLAGS="--sysroot=${STAGING_DIR} -target ${SWIFT_TARGET_NAME} -march=armv7-a -mthumb -mfpu=neon -mfloat-abi=hard"
...
printf "set(CMAKE_SYSTEM_PROCESSOR armv7-a)\n" >> ${SWIFT_CMAKE_TOOLCHAIN_FILE}
printf "set(CMAKE_C_COMPILER $SWIFT_NATIVE_PATH/clang)\n" >> ${SWIFT_CMAKE_TOOLCHAIN_FILE}
printf "set(CMAKE_CXX_COMPILER ${SWIFT_NATIVE_PATH}/clang++)\n" >> ${SWIFT_CMAKE_TOOLCHAIN_FILE}
printf "set(CMAKE_C_FLAGS \"${RUNTIME_FLAGS} ${EXTRA_INCLUDE_FLAGS}\")\n" >> ${SWIFT_CMAKE_TOOLCHAIN_FILE}
printf "set(CMAKE_C_LINK_FLAGS \"${LINK_FLAGS}\")\n" >> ${SWIFT_CMAKE_TOOLCHAIN_FILE}
printf "set(CMAKE_CXX_FLAGS \"${RUNTIME_FLAGS} ${EXTRA_INCLUDE_FLAGS}\")\n" >> ${SWIFT_CMAKE_TOOLCHAIN_FILE}
printf "set(CMAKE_CXX_LINK_FLAGS \"${LINK_FLAGS}\")\n" >> ${SWIFT_CMAKE_TOOLCHAIN_FILE}
...
printf "set(SWIFT_LINUX_${SWIFT_TARGET_ARCH}_ICU_I18N ${STAGING_DIR}/usr/lib/arm-linux-gnueabihf/libicui18n.so)\n" >> ${SWIFT_CMAKE_TOOLCHAIN_FILE}
